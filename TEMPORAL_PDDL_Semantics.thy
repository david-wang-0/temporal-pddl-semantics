...
| Durative_Action_Schema
    (name: name)
    (parameters: "(variable \<times> type) list")                             
    (duration_constraint: "term duration_constraint")
...
context ast_domain begin
  ...
  text \<open>To instantiate an action schema, we first compute a substitution from
    ...
    \<close>
  fun instantiate_action_schema :: "ast_action_schema \<Rightarrow> object list \<Rightarrow> ground_action" where
    "instantiate_action_schema (Simple_Action_Schema n params pre eff) args = 
      (let
  ...
  fun inst_snap_action :: "ast_action_schema \<Rightarrow> object list \<Rightarrow> temporal_annotation \<Rightarrow> ground_action" where
    "inst_snap_action (Durative_Action_Schema n params d cond eff) args ta = 
      ...
      in 
        Ground_Action pre\<^sub>i eff\<^sub>i
      )"
...
context ast_problem begin
  ...
  fun res_inst :: "plan_action \<rightharpoonup> ground_action" where
    "res_inst (Simple_Plan_Action n args) =
      Some (instantiate_action_schema (the (resolve_action_schema n)) args)"
  | "res_inst (Durative_Plan_Action _ _ _) = None"
  ...
  fun wf_plan_action :: "plan_action \<Rightarrow> bool" where
    ...
    | "wf_plan_action (Durative_Plan_Action n args d) = (
      ...
      | Some (Durative_Action_Schema n params dconst cond eff) \<Rightarrow> 
          ...
          \<and> duration_matches d dconst params args \<and> d \<ge> 0)"
  ...
  definition ind_happ_seq :: "plan \<Rightarrow> happening list \<Rightarrow> bool" where
    "ind_happ_seq \<pi>s hs \<longleftrightarrow> (
      ...
      \<and> (\<forall>(t\<^sub>\<pi>,\<pi>) \<in> simple_acts \<pi>s. 
          let g\<^sub>a = the (res_inst \<pi>) in
          \<exists>A. (t\<^sub>\<pi>,A) \<in> set hs \<and> g\<^sub>a \<in> set A) 
...
lemma ind_happ_seq_props:
  ...
  shows "strict_sorted (map fst hs)"
        "(t\<^sub>\<pi>,\<pi>) \<in> simple_acts \<pi>s \<Longrightarrow> \<exists>A. (t\<^sub>\<pi>,A) \<in> set hs \<and> the (res_inst \<pi>) \<in> set A"
...
context ast_problem begin
  ...
  fun wf_ground_action :: "ground_action \<Rightarrow> bool" where
    "wf_ground_action (Ground_Action pre eff) \<longleftrightarrow> (wf_fmla objT pre \<and> wf_effect objT eff)"
...
context wf_ast_problem begin
  ...
  proof -
    ...
    obtain ta where "res_inst \<pi> = Some a \<or> res_inst_snap_action \<pi> ta = Some a"
      using assms by auto
    then show ?thesis
    proof (elim disjE)
      assume case_smpl_act: "res_inst \<pi> = Some a"
      then show ?thesis 
        proof (cases \<pi>)
  ...
  proof -
    ...
    then obtain t\<^sub>\<pi> \<pi> where "(t\<^sub>\<pi>,\<pi>) \<in> set \<pi>s" and "inst_of_plan_action \<pi>s (t\<^sub>\<pi>,\<pi>) (t\<^sub>i,a)"
      by auto
    then show ?thesis 
...
lemma inst_of_plan_actionE:
  "\<And>t\<^sub>\<pi> \<pi>. \<lbrakk>inst_of_plan_action \<pi>s (t\<^sub>\<pi>,\<pi>) (t\<^sub>a,a);
           \<lbrakk>res_inst \<pi> = Some a; t\<^sub>\<pi> = t\<^sub>a\<rbrakk> \<Longrightarrow> Q;
  ...
  lemma ind_happ_seq_happ_at_htps:
    ...
    shows "a \<in> set A'"
  proof-
    obtain t\<^sub>\<pi> \<pi> where "(t\<^sub>\<pi>, \<pi>)\<in>set \<pi>s"
      "(res_inst \<pi> = Some a \<and> t\<^sub>\<pi> = t)
    ...
    then obtain A'' where "(t,A'') \<in> set hs'" "a \<in> set A''"
      using ind_happ_seq_props(2-4)[OF \<open>ind_happ_seq \<pi>s hs'\<close>]
      by (force simp: durative_acts_def simple_acts_def is_act_simple_def split: plan_action.splits)
    moreover hence "A'' = A'"
